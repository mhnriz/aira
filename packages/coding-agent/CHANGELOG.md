...
- **Aira**: Native Workbench & UI overhaul (Phase 12): aira-zhr is now the built-in default dark theme (light stays the light default; explicitly configured themes win), with eight optional semantic color roles (`copper`, `copperBright`, `blue`, `cyan`, `green`, `yellow`, `red`, `purple`) that fall back to classic equivalents so third-party themes stay valid untouched.

- Added the native Workbench: a pure projection layer (`src/aira/ui/`, headless-safe, token-free) renders the engineering sidebar and the status rail from canonical snapshots only. Panels follow an explicit P0–P3 priority model with progressive disclosure: Interaction/Permission (pending ask_user or permission ASK rises to the top), Current Finding (arbitrated across the verifier, LSP/intelligence, browser, execution, orchestration, goal, and interaction sources — severity comes only from the source), Verification (FAIL/INCONCLUSIVE prominent, stale represented honestly), Goal, Tasks, Agents, Execution (including failed background processes), Browser, Working Set, Relevant Symbols, Changeset, Intelligence, and Control (P3, dropped in medium layouts).

- Added the responsive Workbench sidebar and footer: visible by default at safe widths, auto-hidden below `72 + sidebarWidth` columns, explicit OFF respected, restore semantics deterministic; fullscreen mode uses a native HStack split with independent sidebar scrolling, regular mode a viewport-fixed overlay rail (`AiraTuiMainScreen`). The footer is a single-line segment rail (mode/context/model never dropped; git delta, permission mode, LSP detail drop first) that also surfaces the single highest-priority current finding. New canonical seams: intelligence `subscribe()` + bounded top findings + working-set/relevant-symbols queries.

- Added Workbench input and control: `Ctrl+Shift+O` toggles the sidebar (`app.workbench.toggle`) while `Ctrl+O` remains tool-output expansion (`app.tools.expand`; user customizations are never overwritten and `/doctor` reports the resolved truth), the `/workbench [on|off]` command, four settings (`workbench.enabled|showOnStartup|density|width`), startup/keybinding/theme docs, and `/doctor` checks for the workbench binding, aira-zhr availability, sidebar health, and extension chrome conflicts (a custom extension footer suppresses the native rail and is reported truthfully).

- Polished native Workbench visual parity: continuous muted pane separation, copper identity, width-stable aligned rows, bounded diagnostic detail with freshness honesty, distinct task and agent lifecycle panels, cold-state suppression, and footer priorities applied by the native renderer at narrow widths.

- Replaced the stock interactive startup presentation with an Aira-owned application shell: restrained identity/project/model header, explicit conversation roles, compact notices and activity rows, framed bottom composer, sparse-session dock anchoring, one telemetry footer, and bounded Engineering Context overview panels. Existing editor, transcript, tools, extensions, queueing, session, keybinding, fullscreen, headless, and RPC behavior is preserved.

- **Aira**: Native multi-pane viewports (Phase 12.1). Interactive mode defaults to the fullscreen viewport shell (`tuiMode` unset now resolves to `fullscreen`; regular remains the explicit compatibility mode via settings or `--tui-mode regular`). The conversation and the Workbench are two independent scroll views inside one fixed shell — the Aira header, composer dock, and status rail never move while either pane scrolls. The conversation pane gained a fixed `CONVERSATION` title strip; the Workbench's `ENGINEERING CONTEXT` title strip is now a fixed pane header above the scrollable panels.

- Added keyboard viewport focus (`app.viewport.focusCycle`, default `Alt+O`): unmodified PageUp/PageDown/Home/End/line scroll target the focused pane (conversation by default; `Alt+O` cycles to the Workbench). A hidden Workbench cannot take focus and releases it to the conversation when it auto-hides; `Ctrl+A` stays editor line-start, `Ctrl+O` stays tool expansion, `Ctrl+Shift+O` stays the Workbench toggle. Mouse/trackpad wheel always scrolls the pane under the pointer without changing focus. `/doctor` gained a `viewport focus shortcut` check.

- Added a one-line new-output indicator (`↓ N new lines`) when the conversation is scrolled away from live output — it occupies no layout space when idle and clears on End/jump-to-bottom, which also restores follow mode. The Workbench title subtitle reports `LIVE STATE · VIEWING HISTORY` while the transcript reads history and `CANONICAL STATE · TOKEN-FREE` at the live bottom. Subtle copper focus marks identify the focused pane. Viewport state is interactive-only (never `AiraSessionState`); headless/SDK/RPC are unaffected and rendering remains token-free.

- **Aira**: Native interaction & control layer (Phase 11): the deterministic permission pipeline, structured Q&A, and the canonical task graph — one native human-control surface over the Phase 3-10 systems.

- Added the native permission controller (`src/aira/permissions/`, ADR-030): a deterministic authorization pipeline in the host tool boundary (`AgentSession.beforeToolCall`) layered on the Phase 5 capability contract. Precedence is documented and fixed: the PLAN read-only boundary is absolute (no permission mode or rule can weaken it; PLAN + yolo + mutation is still denied), then "allow once" grants (consumed by the very request they approved), then explicit rules (most specific match wins — exact subject beats wildcard, most-recent rule breaks ties; persistent rules live in Aira-owned `~/.aira/agent/permissions.json`, project config is NEVER read so a repository cannot self-grant privileges), then permission-mode defaults. Four small native modes: `normal` (routine engineering allowed; risky/destructive process commands, out-of-workspace writes, network and unknown tools ask), `permissive` (normal asks auto-approve; explicit ask rules still prompt), `strict` (deny-unapproved — only read-only/diagnostic/orchestration/interaction classes and browser observation stay allowed by default), `yolo` (bypass — every ask auto-approves; explicit deny rules and PLAN remain absolute). The risk-marker table (git push/commit/reset, dependency installs, sudo/destructive shell, curl|sh, publish, shutdown, secret-adjacent reads) is a bounded deterministic substring list over the normalized command.

- Added the native structured Q&A manager (`src/aira/interaction/`): ONE pending interaction per session, shared by permission ASK (type "permission") and the model-facing `ask_user` tool (type "semantic") — no second permission-dialog architecture, no duplicate Q&A store. Bounded schemas (question, short context, up to 12 choices with descriptions, single/multi-select, freeform), truthful resolution (answered / cancelled / timed-out / unavailable / superseded), cancellations never fabricate answers, session shutdown resolves pending questions without wedging, and headless sessions resolve as "unavailable" without hanging. The interactive TUI renders pending questions through the existing selector/input dialogs and delivers answers back (attachUI bridge).

- Added the canonical task graph (`src/aira/tasks/`, one owner per session): forward-only validated transitions (pending → active → completed; cancelled; failed for child rows), `blocked` DERIVED from unfinished dependencies (never settable, and dependency-blocked tasks reject activation truthfully), single-task patching (never full-list replacement), bounded rows with oldest-settled eviction, and orchestration child runs projected as read-only rows through the Phase 9 event seam (patching them is refused — orchestration stays the owner of child run records; manual/model tasks never silently become a second task universe). The model-facing `tasks` tool (create/patch/list/get/remove) and the restrained `/tasks` surface (add/done/cancel/get/remove) expose it.

- Added Phase 11 goal integration: an open question moves an active goal to `waiting` with a structured kind (`user-question` / `permission` / `evidence` — never inferred from strings); a real answer resumes the goal (waiting → active), a cancelled semantic question leaves the goal waiting truthfully (no invented answer), a permission denial resumes the round with the truthful denial. Children never prompt: deterministic child tool gating evaluates the root permission policy with ask→deny (nested interactive storms are impossible by construction).

- Added canonical Phase 11 settings (`permissions.enabled`/`permissions.mode`, `interaction.timeoutMs`, `tasks.enabled`) through the `/settings` menu ("Aira permissions" / "Aira interaction" / "Aira tasks"), the restrained `/permissions` command (status, mode, rule add/list/remove), the `/status` appendices (permissions/interaction/tasks lines), and the `/doctor` checks (permissions incl. a PLAN-absolute proof across every mode, interaction pending truth, task graph health). The `ask_user` and `tasks` tools classify as the new `interaction` capability class (never workspace-mutating, available in PLAN as read-only-mode activities, never granted to children). All new state (`state.permissions`, `state.interaction`, `state.tasks`) is bounded, token-free, UI-ready, and subscription-visible — the future Workbench/footer consumes it without model tokens.
...
- Added the native orchestration scheduler: a small DAG engine (one batch ≤ 8 tasks, same-batch `dependencies`) with DFS cycle detection and clear rejection of duplicate ids, self/unknown dependencies, and oversized batches BEFORE dispatch; a semaphore bounds parallel children (`orchestration.maxParallel`, default 2) with queued wait phases (waiting-dependency / waiting-capacity); a failed upstream dependency rejects its dependents synthetically (category `dependency-failed`, never a hang). Cancellation is first-class: per-run and per-batch AbortSignals propagate into the child model stream; timeout (default 5 min, per-task override, hard bounds), user `/agents cancel` / `agents_cancel`, and session dispose all settle runs truthfully (`cancelled` / `timed-out`) with no orphan children or dangling timers.

- Added truthful model selection for children: per-task explicit "provider/model" override, else the `orchestration.model` policy (inherit = current session model, default = configured default model, or an explicit selector). An unavailable or unauthenticated model fails by name (`model-unavailable` category) and is never silently substituted; the resolved model identity is recorded per run and surfaced in telemetry. Real provider token usage (`AssistantMessage.usage`) is recorded per child and aggregated in canonical state when the provider exposes it — never invented.

- Added canonical orchestration state (`state.orchestration`): a bounded, token-free, TUI-independent snapshot (status idle/active, runningCount, queuedCount, maxConcurrency, ≤ 12 child rows with role/task/status/phase/model/elapsed/dependencies/resultSummary/tokenUsage/bounded failure info, ≤ 8 recent results, ≤ 6 failure rows with category/message/timestamp/retryable, aggregate token usage, one-line summary) published by the manager with a subscribe seam — the future Workbench/footer rendering the `◇ AGENTS 3` chip (UI_BACKLOG B-004) needs no child processes, logs, or model tokens.

- Added the native delegation tools `agents_delegate` (batch dispatch, await or background), `agents_status` (bounded snapshot), and `agents_cancel` (one run or all), registered and active by default (SDK default tool list included); they classify as the new `orchestration` capability class, which host-level PLAN policy does not block — the scheduler's mode gate IS the read-only enforcement point: mutation-capable roles (implement/test) are refused per-task in PLAN with a truthful reason, read-only roles still dispatch with mode-collapsed tool sets, and a PLAN child physically cannot write/execute/browse (proven by host tests and real-binary dogfood).

- Added canonical orchestration settings (`orchestration.enabled` default true, `orchestration.maxParallel` 1-8 default 2, `orchestration.model` inherit/default/"provider/model" default inherit, `orchestration.timeoutMs` 30s-15m default 5m) surfaced through the `/settings` menu ("Aira agents"), plus the restrained `/agents` status/inspect and `/agents cancel [id|all]` command surface, the `/status` orchestration line, and the `/doctor` orchestration check (reports enabled/disabled, state, concurrency, active children, failure count — never dispatches; a disabled feature is reported truthfully, not as broken).

- Added the native Aira browser runtime (Phase 7): a per-session browser subsystem (`src/aira/browser/`) with one runtime owner behind a replaceable provider boundary (ADR-025). The default provider is an Aira-native CDP/Chromium implementation (zero new npm dependencies; native WebSocket) that launches an ISOLATED disposable browser (headless, fresh profile under the canonical cache, random loopback DevTools port) and never attaches to a personal browser profile. It provides bounded semantic observation (accessibility-tree outline with stable element refs, char/node budgets), ref-based interaction (click/fill/press/scroll/wait/evaluate), bounded deduplicated console and network-failure evidence (counts + top finding), screenshots to managed paths (pruned, metadata-only in state), ref staleness that never silently re-targets a different element, truthful degraded/unavailable states, and cleanup that kills only Aira-owned processes (crash-path pid tracking included).

- Added the native Aira independent verification subsystem (Phase 8): a per-session verification manager (`src/aira/verification/`, ADR-026) that verifies non-trivial engineering completions at the `agent_end` completion boundary through a FRESH-CONTEXT verifier model call (its own verifier-role system prompt, a bounded secret-redacted evidence envelope, restricted read-only tools `read`/`grep`/`find`/`ls`, at most two tool rounds). The implementing agent's conversation is never included, which is the documented independence boundary. Verdicts are requirement-driven and canonical: PASS / FAIL / INCONCLUSIVE with per-requirement verified/unmet/unverifiable status, findings, evidence items, scope assessment (in-scope/drift/uncertain), missing evidence, and confidence. Hardening rules keep INCONCLUSIVE honest: a pass with unmet requirements is FAIL, a pass without concrete evidence is INCONCLUSIVE, malformed verdicts and verifier driver failures (model/auth/timeout/cancellation) are INCONCLUSIVE with an explicit lastError — never a silent PASS.

- Added verification settings (`verification.enabled` default true, `verification.auto` off|smart|always default smart, `verification.contextBudget` compact|balanced|expanded default compact) through the canonical `/settings` surface. Smart mode skips trivial work (docs/comments/one-line renames with clean diagnostics) without invoking the verifier model; always verifies every meaningful implementation; off never auto-verifies (explicit `/verify` still works while enabled). Evidence-envelope budgets are hard (compact ≤ 6000 / balanced ≤ 10000 / expanded ≤ 16000 chars).

- Added canonical verification state (`state.verification`): a bounded, token-free, TUI-independent snapshot (status idle/preparing/running/passed/failed/inconclusive, currentResult, requirement counts, highest finding, stale flag, missing evidence, lastError, lastSkipReason) published by the manager with a subscribe seam — the future Workbench/footer render it without model tokens. Revision-hash dedupe means unchanged implementations are not reverified; a new relevant edit or a moved change set invalidates a prior result immediately (event-driven) and change-set mtime drift stales on refresh; a stale PASS is not completion evidence. The completion fence marks a result stale if the change set moves while the verifier runs. No repair loop exists: FAIL carries structured findings for a later orchestrator.

- Added the restrained `/verify` (explicit run; reuses current results for unchanged revisions, honors `verification.enabled`) and `/verify status` (renders the canonical snapshot) commands, the `/doctor` verifier check (configuration/health/verdict — never runs the model), and the restrained `/status` verification line.

- Added the native browser model tools (`browser_open`, `browser_status`, `browser_observe`, `browser_navigate`, `browser_click`, `browser_fill`, `browser_press`, `browser_scroll`, `browser_wait`, `browser_evaluate`, `browser_console`, `browser_network`, `browser_screenshot`, `browser_verify`, `browser_close`) plus the restrained `/browser` command, and the canonical `browser.*` settings (`enabled`, `context` off|auto|on, `autoVerify`, `contextBudget`) surfaced through the existing `/settings` menu. Browser tools classify as `browser` with semantic operation kinds: PLAN keeps observation/navigation and blocks interact/lifecycle. Ambient context respects hard budgets and content-hash deduplication; AUTO commonly injects zero browser tokens while `state.browser` remains UI-visible and token-free. `/doctor` gained a `browser` check and `/status` a restrained `browser:` line.

- Added the native Aira execution runtime (Phase 6): a per-session process manager (`src/aira/execution/`) that launches foreground/background/auto commands, captures bounded stdout/stderr tails (128 KiB per stream, truncation flagged), tracks truthful status and exit codes, terminates gracefully with a forced group/tree fallback (POSIX signals / Windows taskkill, including survivor reaping), cleans up the session's own processes on disposal, supports conservative dev-server reuse, and publishes a bounded execution snapshot into canonical session state (`/doctor` execution check). Process ownership is per-session-instance, so overlapping sessions over the same session file cannot kill each other's processes (ADR-024).

- Added the native process model tools (`process_start`, `process_status`, `process_logs`, `process_stop`) and the restrained `/processes` command. The tools are registered and active by default; `process_start`/`process_stop` classify as `process` (PLAN-blocked) and `process_status`/`process_logs` as `diagnostic` (safe in PLAN) under the semantic capability contract. The default SDK tool list now includes them (explicit `defaultTools`/`tools` selections still win).

- Added project-aware execution primitives consuming the canonical project profile: test runs at TARGETED (toolchain-aware target suffix) / RELATED (nearest subpackage profile) / PROJECT depth, plus build, check (new conventional check commands in the profile: tsc/scripts.check, mypy, ruff, cargo check, go vet; build fallback), and dev (background by default, reuse-aware) commands.

- Added the future-UI backlog for ambient language-intelligence/LSP state in the bottom bar (`aira_product_docs/UI_BACKLOG.md`, B-001). Recorded only; not implemented.

- Added native Aira intelligence (Phase 5): an ambient intelligence service arming from the canonical project profile. A bounded repository provider indexes file-level evidence (languages, symbols, imports, source/test counterparts, git changed files) with a JSON cache under the Aira home; a live-code provider runs project-scoped language servers (TypeScript/JavaScript, Python, Go, Rust, C/C++, C#) behind a minimal JSON-RPC LSP client with post-edit diagnostics, warm-only navigation, idle eviction, and crash degradation; a coordinator injects a bounded, freshness-aware context message (likely files, changed files, diagnostics, impact) at prompt time, runs automatic post-edit diagnostics, and publishes a health snapshot into canonical session state surfaced through `/doctor`. Missing language servers and failed scans degrade to plain Pi behavior.

- Added the native semantic capability classification contract: built-in tools classify as read-only / mutating / process (with diagnostic / network / browser reserved), the PLAN gate derives from it, and unknown third-party tools remain unclassified (Plan mode compatibility preserved). `/doctor` gained a `capabilities` check and an `intelligence` check.
...
- **Aira**: Re-paletted the Workbench chrome with a background-independent semantic palette. `aira-zhr` now uses warm copper `#D89A72` (identity, active mode, focused composer border), muted blue `#7895BC` (LSP/info/links/model), soft blue `#A9BBD3` (active agents), warm ivory `#E7E1D9` text, taupe `#8F8C89` muted, `#82B792` success, `#D6AA6C` warning, `#D77C78` error, and `#66574F` borders. Aira owns foreground semantics; the terminal owns the canvas: the composer input lane and the footer status rail no longer paint gray background fills and sit directly on the terminal background on any dark terminal (black, charcoal, warm-dark, blue-black). The composer frame is copper while focused and muted-subtle otherwise; the footer model segment moved from purple to the restrained informational blue, while permission/control keeps purple. Explicitly configured and third-party themes are untouched (roles keep their classic fallbacks; those themes keep their own fills and colors).

- **Aira**: Versioning policy — Aira's own product version (`AIRA_VERSION`, currently 0.1.2) tracks updates; the Pi base version stays internal for syncing Aira's backend with upstream Pi bug/security fixes. The interactive startup update notice checks Aira's GitHub releases against `AIRA_VERSION` instead of the upstream pi.dev marker, so it no longer reports unrelated upstream Pi versions (e.g. "new version 0.84.4 is available" when the installed Aira build is already current). The notice links to the Aira release page and drops the stale "Run `aira update`" npm instruction.
...
- Fixed the macOS browser launch popup during automation runs ("A keychain cannot be found to store 'Chrome'"): the isolated browser now launches with `--use-mock-keychain` on macOS, replacing Safe Storage key handling with an in-memory mock — headless CI/automation environments with a locked or missing login keychain no longer stall on an OS dialog. Verified in the real-Chrome suite and against the launched process args.
