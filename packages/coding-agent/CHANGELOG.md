...
- Added the native Workbench: a pure projection layer (`src/aira/ui/`, headless-safe, token-free) renders the engineering sidebar and the status rail from canonical snapshots only. Panels follow an explicit P0–P3 priority model with progressive disclosure: Interaction/Authorization (pending ask_user or permission ASK rises to the top), Current Finding (arbitrated across the verifier, LSP/intelligence, browser, execution, orchestration, goal, and interaction sources — severity comes only from the source), Verification (FAIL/INCONCLUSIVE prominent, stale represented honestly), Goal, Tasks & Agents, Execution (including failed background processes), Browser, Working Set, Relevant Symbols, Changeset, Intelligence, and Control (P3, dropped in medium layouts).
...
- Added Workbench input and control: `Ctrl+O` toggles the sidebar (`app.workbench.toggle`; tool-output expansion moved to `Alt+O` — user customizations are never overwritten, `/doctor` reports the resolved truth), the `/workbench [on|off]` command, four settings (`workbench.enabled|showOnStartup|density|width`), startup/keybinding/theme docs, and `/doctor` checks for the workbench binding, aira-zhr availability, sidebar health, and extension chrome conflicts (a custom extension footer suppresses the native rail and is reported truthfully).
