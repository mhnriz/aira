...
Edit directly or use `/settings` for common options. To save startup model defaults interactively, use `/model` and press Ctrl+S on the desired model. Press Ctrl+S in `/thinking` to save the startup thinking level.
...
|---------|------|---------|-------------|
| `defaultProvider` | string | - | Startup provider (saved with Ctrl+S in `/model`, or edited manually) |
