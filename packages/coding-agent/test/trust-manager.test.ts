...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".aira", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".aira", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".aira", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".aira"), { recursive: true });
			writeFileSync(join(cwd, ".aira", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".aira"), { recursive: true, force: true });
