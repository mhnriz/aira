...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".aira", "prompts");
			...
			const projectSkillDir = join(cwd, ".aira", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".aira", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".aira", "themes"), { recursive: true });
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".aira"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".aira", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".aira", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".aira", "extensions");
...
export default function(pi) {
	...
	pi.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".aira", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.aira", async () => {
			const configDir = join(cwd, ".aira");
...
Project skill content`,
	...
	describe("system prompt sources", () => {
		it("exposes discovered project SYSTEM.md as the system prompt source", async () => {
			const configDir = join(cwd, ".aira");
			const systemPromptPath = join(configDir, "SYSTEM.md");
			mkdirSync(configDir, { recursive: true });
		...
		it("exposes discovered APPEND_SYSTEM.md as an append system prompt source", async () => {
			const configDir = join(cwd, ".aira");
			const appendSystemPromptPath = join(configDir, "APPEND_SYSTEM.md");
			mkdirSync(configDir, { recursive: true });
